...
**定位**：一个简洁的 Python 日志服务，适合临时接收、查看或调试外部请求。
...
**目录与模块**：`server.py` 是服务入口；`response.txt` 可作为固定响应内容或样例。

**使用方式**：使用 `uv run server.py` 启动脚本，根据源码中的端口和处理逻辑发送请求。

**配置说明**：当前无独立配置文件，端口和响应逻辑主要在 `server.py` 中。

**适用场景**：适合 webhook 调试、请求日志采集、接口联调和本地 mock。

**注意事项**：日志服务可能记录敏感请求头或正文，公开部署前应增加鉴权和脱敏。
...
2. 启动服务：
   ...
   - `--max-log-body-chars` 可选：日志中正文/命令显示的最大字符数，默认 1000；设置为 0 或负数表示不截断。
...
- 当正文超过 `--max-log-body-chars` 时，日志中的正文和重放命令会被截断并提示 total 字符数；需要完整内容时可调大或关闭截断（设为 0 或负数）。
